...
export interface Trrack<State, Event extends string> {
    ...
    apply<T extends string, Payload = any>(label: string, act: PayloadAction<Payload, T>): any;
    to(node: NodeId): Promise<void>;
    metadata: {
        add(metadata: Record<string, unknown>, node?: NodeId): void;
        latestOfType<T = unknown>(type: string, node?: NodeId): Metadata<T> | undefined;
        allOfType<T = unknown>(type: string, node?: NodeId): Metadata<T>[] | undefined;
    ...
    currentChange(listener: CurrentChangeHandler, skipOnNew?: boolean): UnsubscribeCurrentChangeListener;
    done(): void;
    tree(): any;
